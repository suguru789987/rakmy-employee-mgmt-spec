...
- **従業員登録 (`employee_register.html`)**: 基本情報フォームに **適用日付**（カレンダー、必須、既定=当日）を追加。未来日＝予約登録、当日＝即時登録。
- **従業員詳細 (`employee_detail.html`)**:
  - フッターの **「更新予約」ボタンを廃止**（`#modal_schedule_update` モーダルも削除）。
  - フッターに **適用日付**（カレンダー、必須、既定=当日）を追加。
  - 主ボタンを「保存・即時更新」→ **「保存」** に変更。保存時に適用日付を判定：未来日＝「{日付}からの適用を予約しました」、当日・過去＝「即時反映しました」。
- 設定変更履歴の「ステータス」列は 適用中/予約中/過去 で、適用日付が未来のレコード＝予約中として表示。

### 影響
- ファイル: `employee_register.html`（適用日付追加）、`employee_detail.html`（更新予約廃止＋適用日付）、`employee_payroll_histories.html`（新規）、`sidebar_company.html`（導線）。
