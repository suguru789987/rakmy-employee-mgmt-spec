...
def c6_data():
    ...
    for e in emp.values():
        ...
        d = int(e['想定労働日数'])
        if d > 31:
            bad.append(f"{e['従業員コード']}:想定労働日数{d}日は暦日を超える")
        h = int(e['所定労働時間']) / d
        if not (1 <= h <= 12):
            bad.append(f"{e['従業員コード']}:1日{h:.1f}時間（所定{e['所定労働時間']}÷{d}）")
    check('6 データ', '1日の想定勤務時間＝所定労働時間÷想定労働日数 が1〜12時間（判断26）',
          not bad, bad[:5])
...
def c7_help():
    ...
    NG = ['会社スコープの機能です', 'BOM付き', 'バッジ濃色', '移動元',
          '設定を変更する入口は2つ',                     # J-33：設定は従業員詳細だけ
          '「従業員設定」→ 修正',                        # 同上
          '日別実績に概算モードはございません',           # J-21：日別にも概算モードがある
          '別の列で表示しております']                     # J-14：交通費の列は無い
...
def c12_derived():
    ...
    KEEP = {'名前', '従業員設定', '従業員コード', '年月', '日時', '雇用区分',
